...
export default memo(() => {
  ...
  const onSaveEdit = () => {
    ...
    setdoubleInvestAmount(formatNumber((Number(investAmount) * 3)));
    setdisplayInvestAmount(formatNumber((Number(investAmount))));
    setYears(Math.floor(114/Number(interestRate)).toString());
  };
...
<Text
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.boxInput}>
          ...
          A simple formula that is popularly used to estimate the number of years to triple the invested money at a given rate of interest or returns.
        </Text>
            <View>
              <Text>Amount</Text>
              <TextInput
