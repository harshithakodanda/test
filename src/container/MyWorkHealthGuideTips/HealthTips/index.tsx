...
export default memo(() => {
  ...
  const onSaveEdit = () => {
    ...
    setdoubleInvestAmount(formatNumber((Number(investAmount) * 4)));
    setdisplayInvestAmount(formatNumber((Number(investAmount))));
    setYears(Math.floor(144/Number(interestRate)).toString());
  };
...
<Text
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.boxInput}>
          ...
          A simple formula that is popularly used to estimate the number of years to Quadruple the invested money at a given rate of interest or returns.
        </Text>
            <View>
              <Text>Amount</Text>
              <TextInput
